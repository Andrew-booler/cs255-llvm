...
void count_dynamic_bbs(Function &F){
	...
	for (Function::iterator BB = F.begin(), E = F.end(); BB != E; ++BB) {	
		...
		while (InsertPos!=BB->end())
		{
			if(isa<AllocaInst>(InsertPos) || isa<PHINode>(InsertPos)) {
				InsertPos++;
			}else{
...
void add_init_call(Function &F){
	if(F.getName().equals("main")){
		...
		LLVMContext &Context = F.getContext();
                Type *VoidTy = Type::getVoidTy(Context);
                Constant *IncFn = M->getOrInsertFunction("init", VoidTy, (Type *)0);
		CallInst::Create(IncFn,  "", &*InsertPos);
	}
...
static RegisterPass<InstCounter> X("inst-count", "Instruction Counter Pass");
